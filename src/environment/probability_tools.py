...
from .base import Card, CardDeck, CardHand


@lru_cache
def __simulate_dealer(deck: CardDeck, hand: CardHand, hit_on_soft_17: bool) -> tuple[int, int]:
    if hand.total >= 17 and not (hit_on_soft_17 and hand.is_soft):
        return (1 if hand.total > 21 else 0), 1
...
@lru_cache
def calculate_dealer_will_take_cards_probability(deck: CardDeck, dealer_open_card: Card, hit_on_soft_17: bool = False) -> float:
    if hit_on_soft_17:
...
@lru_cache
def calculate_dealer_busting_probability(deck: CardDeck, open_card: Card, hit_on_soft_17: bool = False) -> float:
    total_bust = 0
    total_possibilities = 0
